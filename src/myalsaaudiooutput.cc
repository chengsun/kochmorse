...
#include <QtMultimedia/5.13.2/QtMultimedia/private/qaudiohelpers_p.h>
#include "myalsaaudiooutput.hh"
#include "myalsaaudiodeviceinfo.hh"

//#define DEBUG_AUDIO 1

MyAlsaAudioOutput::MyAlsaAudioOutput(const QByteArray &device)
{
    bytesAvailable = 0;
    handle = 0;
...
MyAlsaAudioOutput::~MyAlsaAudioOutput()
{
    close();
...
void MyAlsaAudioOutput::setVolume(qreal vol)
{
    m_volume = vol;
}

qreal MyAlsaAudioOutput::volume() const
{
    return m_volume;
}

QAudio::Error MyAlsaAudioOutput::error() const
{
    return errorState;
}

QAudio::State MyAlsaAudioOutput::state() const
{
    return deviceState;
}

int MyAlsaAudioOutput::xrun_recovery(int err)
{
    int  count = 0;
    bool reset = false;
...
int MyAlsaAudioOutput::setFormat()
{
...
void MyAlsaAudioOutput::start(QIODevice* device)
{
...
QIODevice* MyAlsaAudioOutput::start()
{
    if(deviceState != QAudio::StoppedState)
        deviceState = QAudio::StoppedState;
...
int MyAlsaAudioOutput::bytesFree() const
{
    if(resuming)
        return period_size;
...
qint64 MyAlsaAudioOutput::write( const char *data, qint64 len )
{
...
int MyAlsaAudioOutput::periodSize() const
{
    return period_size;
}

void MyAlsaAudioOutput::setBufferSize(int value)
{
    if(deviceState == QAudio::StoppedState)
        buffer_size = value;
}

int MyAlsaAudioOutput::bufferSize() const
{
    return buffer_size;
}

void MyAlsaAudioOutput::setNotifyInterval(int ms)
{
    intervalTime = qMax(0, ms);
}

int MyAlsaAudioOutput::notifyInterval() const
{
    return intervalTime;
}

qint64 MyAlsaAudioOutput::processedUSecs() const
{
    return qint64(1000000) * totalTimeValue / settings.sampleRate();
}

void MyAlsaAudioOutput::resume()
{
...
void MyAlsaAudioOutput::setFormat(const QAudioFormat& fmt)
{
    if (deviceState == QAudio::StoppedState)
        settings = fmt;
}

QAudioFormat MyAlsaAudioOutput::format() const
{
    return settings;
}

void MyAlsaAudioOutput::suspend()
{
...
void MyAlsaAudioOutput::userFeed()
{
...
bool MyAlsaAudioOutput::deviceReady()
{
    if(pullMode) {
        int l = 0;
...
qint64 MyAlsaAudioOutput::elapsedUSecs() const
{
    if (deviceState == QAudio::StoppedState)
        return 0;
...
AlsaOutputPrivate::AlsaOutputPrivate(MyAlsaAudioOutput* audio)
{
    audioDevice = qobject_cast<MyAlsaAudioOutput*>(audio);
}
